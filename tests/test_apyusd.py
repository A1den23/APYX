...
def test_evaluate_total_assets_alerts_on_one_hour_change() -> None:
    ...
    history.record("total_assets:apyUSD", 1000000.0, now - timedelta(minutes=60))
...
def test_evaluate_price_apxusd_alerts_on_one_hour_change() -> None:
    history = RollingMetricHistory()
    engine = AlertEngine(cooldown=timedelta(minutes=5))
    now = datetime(2026, 4, 24, 15, 30, tzinfo=timezone.utc)
    history.record("apyusd_price_apxusd", 1.20, now - timedelta(minutes=60))
    ...
    assert "1h change: +11.67%" in event.body
